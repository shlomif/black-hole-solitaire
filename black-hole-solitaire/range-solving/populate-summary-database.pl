...
{
    $dbh->do(
"CREATE TABLE $runs_table (idx INTEGER PRIMARY KEY, status CHAR(1), num_checked INTEGER, num_generated INTEGER)"
    );
    $dbh->do(
"CREATE TABLE $solutions_table (idx INTEGER PRIMARY KEY, solution VARCHAR(150))"
    );
}
...
my $unsolved_sth =
    $dbh->prepare(
"INSERT INTO $runs_table (idx, status, num_checked, num_generated) VALUES (?, 'U', ?, ?)"
    );

my $solved_sth =
    $dbh->prepare(
"INSERT INTO $runs_table (idx, status, num_checked, num_generated) VALUES (?, 'S', ?, ?)"
    );
...
m{^Total number of states checked is (\d+)\.\nThis scan generated (\d+) states\.$}ms
        ...
        else
        {
            ...
            my @moves =
                ( $text =~
m{^Move a card from stack (\d+) to the foundations\n\nInfo: Card moved is ($card_re)\n\n\n====================\n}gms
                );
            ...
            $solution_sth->execute(
                $deal,
                join(
                    "",
                    &zip(
                        [ @stack_to_chr{@$stacks} ],
                        [ @card_str_to_chr{@$cards} ]
                    )
                )
            );
        }
